...
indexGlaucoma= find(~cellfun(@isempty,strfind(T.Notes,'laucoma')).*strcmp(T.Group,'Congenital'))';
...
for i=1:numClust
    ...
    % add an 'x' symbol over the glaucoma subjects
    glaucomaSubJ=find(ismember(indexEarlyBlind,indexGlaucoma));
    plot(1.75+j(glaucomaSubJ),all_score(indexGlaucoma,i),'Marker','x','LineStyle','none','MarkerEdgeColor',[.5 .5 .5],'MarkerFaceColor','none')    
...
for i=1:6
    ...
    % add an 'x' symbol over the glaucoma subjects
    glaucomaSubJ=find(ismember(indexEarlyBlind,indexGlaucoma));
    plot(1.75+j(glaucomaSubJ),othermeasures(indexGlaucoma,i),'Marker','x','LineStyle','none','MarkerEdgeColor',[.5 .5 .5],'MarkerFaceColor','none')    
...
% Report kruskal-wallis test comparing the groups in the age-regressed FA values
...
% Repeat the test, now excluding subjects with congenital glaucoma

fprintf('follow-up test, now excluding subjects with congenital glaucoma:\n');

group=othermeasures(:,1)*nan;
group(indexsight_protocol2)=1;
group(indexEarlyBlind_protocol2)=2;
group(indexGlaucoma)=nan;
group(indexlca1_protocol2)=3;
